...
- `planner.ts` — BattlePlanner: pure functions for the planning phase. `parsePlan()` extracts deliverable/approach/steps from Markdown. `mapStepsToRounds()` distributes steps via ceil(N/M) heuristic. `buildPlanningPrompt()` generates round 0 prompt with deliverable disambiguation. `buildExecutionPrompt()` generates per-round directed prompts.
